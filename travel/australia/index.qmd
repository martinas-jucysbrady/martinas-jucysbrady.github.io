...
<style>
  ...
  .legend-dash.flight { color: #9ab8c9; }
  .legend-dash.drive  { color: #c9aa96; }
</style>
...
<div class="trip-header">
  ...
  <!-- ── Map container (position:relative so sprite can be positioned inside) ── -->
  <div style="max-width:860px; margin: 28px auto 8px; position:relative;">
    <div id="route-map" style="width:100%; height:380px; border-radius:14px; box-shadow:0 4px 24px rgba(0,0,0,0.10);"></div>
    <!-- Animated vehicle — starts hidden, revealed by JS -->
    <div id="vehicle-sprite" style="display:none;">
      <!-- Plane SVG (shown on flight legs) -->
      <svg id="icon-plane" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" style="display:none;">
        <circle cx="32" cy="32" r="32" fill="#1E4264" opacity="0.85"/>
        <text x="32" y="44" text-anchor="middle" font-size="28">✈️</text>
      </svg>
      <!-- Car SVG (shown on drive legs) -->
      <svg id="icon-car" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" style="display:none;">
        <circle cx="32" cy="32" r="32" fill="#c9aa96" opacity="0.85"/>
        <text x="32" y="44" text-anchor="middle" font-size="28">🚗</text>
      </svg>
    </div>
    <p style="font-family:'Cormorant Garamond',serif; font-style:italic; font-size:0.88rem; color:#9b8877; text-align:center; margin-top:10px;">
      Singapore → Perth → Busselton → Pinnacles → Sydney → Bondi → Blue Mountains → Dublin
    </p>
    <div class="map-legend">
      <span class="legend-item"><span class="legend-dash flight"></span>Flight</span>
      <span class="legend-item"><span class="legend-dash drive"></span>Road trip</span>
    </div>
  </div>
...
(function() {
  ...
  function init() {
    ...
    L.tileLayer('https://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}{r}.png',
      { subdomains: 'abcd', maxZoom: 19 }).addTo(rmap);
    L.tileLayer('https://{s}.basemaps.cartocdn.com/light_only_labels/{z}/{x}/{y}{r}.png',
      { subdomains: 'abcd', maxZoom: 19, opacity: 0.4 }).addTo(rmap);
    ...
    function arcPoints(from, to, steps) {
      steps = steps || 50;
      var pts    = [];
      // Pull midpoint "up" (northward) for a natural flight arc
      var midLat = (from[0] + to[0]) / 2 - Math.abs(to[0] - from[0]) * 0.15;
      var midLng = (from[1] + to[1]) / 2;
      for (var i = 0; i <= steps; i++) {
        var t   = i / steps;
        var lat = (1-t)*(1-t)*from[0] + 2*(1-t)*t*midLat + t*t*to[0];
        var lng = (1-t)*(1-t)*from[1] + 2*(1-t)*t*midLng + t*t*to[1];
        pts.push([lat, lng]);
      }
      return pts;
    }
    ...
    // FROM-stop indices that are long-haul flights
    var flightLegs = [0, 5, 9]; // SIN→PER, PER→SYD, SYD→DUB

    /* ── Vehicle sprite refs ── */
    var sprite    = document.getElementById('vehicle-sprite');
    var iconPlane = document.getElementById('icon-plane');
    var iconCar   = document.getElementById('icon-car');

    /* ─────────────────────────────────────────
       Animate vehicle along an array of latlngs
       Returns a Promise that resolves when done.
    ───────────────────────────────────────── */
    function animateVehicle(pts, isFlight) {
      return new Promise(function(resolve) {
        // Show correct icon
        iconPlane.style.display = isFlight ? 'block' : 'none';
        iconCar.style.display   = isFlight ? 'none'  : 'block';
        sprite.style.display    = 'block';

        var step  = 0;
        // Speed: flights take ~1200ms total, drives ~800ms
        var delay = isFlight ? Math.round(1200 / pts.length)
                             : Math.round(800  / pts.length);
        ...
        function tick() {
          ...
          var px = rmap.latLngToContainerPoint(L.latLng(pts[step]));
          sprite.style.left = px.x + 'px';
          sprite.style.top  = px.y + 'px';

          // Rotate plane to face direction of travel
          if (isFlight && step < pts.length - 1) {
            var next  = rmap.latLngToContainerPoint(L.latLng(pts[step + 1]));
            var angle = Math.atan2(next.y - px.y, next.x - px.x) * 180 / Math.PI;
            iconPlane.style.transform = 'rotate(' + (angle - 0) + 'deg)';
          }
    ...
    /* ─────────────────────────────────────────
       Sequential animation: draw leg, move vehicle, drop pin
    ───────────────────────────────────────── */
    var LEG_PAUSE = 200; // ms pause between legs

    function runLegs(i) {
      if (i >= stops.length) return;

      var stop     = stops[i];
      var isFlight = flightLegs.indexOf(i - 1) !== -1;
      var isDrive  = !isFlight && i > 0;

      // Drop pin at current stop immediately (stop 0 = origin)
      if (i === 0) {
        L.marker(stop.latlng, { icon: makePin(stop.icon, stop.label) }).addTo(rmap);
        setTimeout(function() { runLegs(1); }, LEG_PAUSE);
        return;
      }

      var from = stops[i-1].latlng;
      var to   = stop.latlng;
      var pts  = isFlight ? arcPoints(from, to, 60) : [from, to];

      // Draw polyline first
      L.polyline(pts, {
        color:     isFlight ? '#9ab8c9' : '#c9aa96',
        weight:    isFlight ? 1.5 : 2,
        dashArray: isFlight ? '4,5' : '5,4',
        opacity:   0.85
      }).addTo(rmap);

      // Then animate vehicle along it
      animateVehicle(pts, isFlight).then(function() {
        // Drop destination pin
        L.marker(stop.latlng, { icon: makePin(stop.icon, stop.label) }).addTo(rmap);
        setTimeout(function() { runLegs(i + 1); }, LEG_PAUSE);
      });
    }

    // Kick off after a short delay so map tiles load
    setTimeout(function() { runLegs(0); }, 800);
  }
